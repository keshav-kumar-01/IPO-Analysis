...
@st.cache_data
def load_data():
    return pd.read_csv(r"D:\IPO_analysis\data\ipo_features.csv", parse_dates=["Date"])

@st.cache_resource
def load_model():
    with open(r"D:\IPO_analysis\models\best_xgb_model.pkl", "rb") as f:
        return pickle.load(f)   # returns dict {model, features, imputer, scaler}
...
# App Layout
# ----------------------------
st.set_page_config(page_title="IPO Analysis Dashboard", layout="wide")
...
# ==================================================
    ...
    try:
        ...
        ipo_row_proc = imputer.transform(ipo_row)
        ipo_row_proc = scaler.transform(ipo_row_proc)
...
# ==================================================
    ...
    st.write(f"Simulating strategy: {strategy} with capital ₹{capital}")
